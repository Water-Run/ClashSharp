...
Windows 11 Sandbox 已通过生产 parent/helper 的安装、修复和卸载事务，并在服务
就绪修复后完成七个持久阶段终止与一个文件占用恢复场景。安装会完成受认证的服务
Hello 握手；卸载后包、服务、安装器拥有的两类证书及私有运行目录均已移除。
该验证使用一次性客体信任和明确记录的客体目录 ACL 夹具；WPF 页面操作、升级、
跨用户关联与完整故障矩阵仍需完成，开发包不能被提升为 1.0.0 正式发布。
