...
internal static class LogStorageSchema
{
    ...
    public static void EnsureCreated(SqliteConnection connection)
    {
        ...
        ExecuteNonQuery(connection, "CREATE TABLE IF NOT EXISTS TrafficCounterState (Id INTEGER PRIMARY KEY CHECK (Id = 1), Epoch TEXT NOT NULL, UploadBytes INTEGER NOT NULL, DownloadBytes INTEGER NOT NULL);");
        ExecuteNonQuery(connection, "CREATE TABLE IF NOT EXISTS TrafficConnectionCounters (ConnectionId TEXT NOT NULL, StartedAtTicks INTEGER NOT NULL, UploadBytes INTEGER NOT NULL, DownloadBytes INTEGER NOT NULL, PRIMARY KEY (ConnectionId, StartedAtTicks));");
