...
9 月 23 日 17:57 UTC：当前服务器安装 `41344d8`，CI 35896704317 通过，安装 DLL 与候选载荷散列一致。227cb08、3d5bad4、3e30bec、0492ead、fb43a6d、9a0db48 正常卸载后均核实包/服务及六处自有目录消失，随后正常安装成功；验收数据另从关闭后的散列备份恢复，不计自动升级保留数据通过。此前 ef6bfa7 已通过“修复中断且应用包缺失”的真实恢复，未手工注册载荷。41344d8 的俄语启动引导已通过打开期间缩至最小窗口的显示复验；德语、俄语、法语磁贴遍历和编辑器均补入实机证据。新发现首页标题在小窗口被长语言链接挤压，源码修复及 121 项回归完成，等待候选包复验。
...
当前矩阵 **68 项通过、33 项部分完成、40 项待测**；完整多语言与 RTL、小窗口/键盘/托盘、采样统计与清理、安装器剩余故障场景和临时资源回收尚未完成，不能作为完整发布验收。17:38 UTC 再核对本地与远端仅有 `main`，开放 PR 为零。生产设置权威切换仍是独立未完成事项。
