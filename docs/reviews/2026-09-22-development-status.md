...
9 月 23 日继续验收：`ccea7ce` 已验证节点显示、TUN 切换和重启后的选择保持。`656418c` 已验证关闭自动更新仍可手动更新、待命首页与托盘状态稳定，以及修复取消后继续；未完成事务使用其他候选时，`4d9b426` 正确说明应使用原安装器。`4d9b426` 已在服务器全新安装，验证备份恢复、订阅用量/到期/零值和窄窗口布局，以及运行中切换配置、更新当前订阅、取消和执行历史恢复。正文下载在响应头之后缺少超时被实机复现，现已补全下载期限；配置操作结果提示和可读历史标签也已补入七种语言，相关 371 项与主程序 3086 项回归通过，等待新包复验。累计流量漏计、其余交互矩阵及资源清理仍未完成，不能作为完整发布验收。
