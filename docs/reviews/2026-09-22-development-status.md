...
9 月 23 日继续验收：`ccea7ce` 已在目标服务器验证节点显示、TUN 开关及正常重启后的选择保持，六份实际网络探测确认全局代理与规则直连符合各自选择。订阅空输入无反馈、无选择按钮误启用和初始日期异常已完成源码修复，主程序 3063 项回归及两项 CI 通过，服务器新包待实测。另复现并修复首页与托盘并发读取导致的短暂“不可用”，143 项相关测试通过，尚待新包稳定性复验。累计流量漏计、完整订阅生命周期、其余交互矩阵及资源清理仍未完成，不能作为完整发布验收。
