...
9 月 23 日继续验收：`9dfbd39` 已在服务器全新安装，App 核心下短连接、一分钟下载、停止和安全退出前最后一笔流量均与核心计数及数据库对齐，重启未重复入账。服务 TUN 的空连接列表和透明代理下载已验证；采样开关、清空历史和分类统计含义仍待覆盖。`6c38c1e` 修复实机发现的触发器直接点保存丢失输入，源码回归与离线安装包 CI 均通过，正在安装复验。此前规则列、重命名、订阅正文超时、历史恢复和配置删除证据继续保留。当前矩阵为 51 项通过、20 项部分完成、70 项待测；其余交互矩阵及资源清理尚未完成，不能作为完整发布验收。
