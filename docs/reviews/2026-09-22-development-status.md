...
9 月 23 日 11:40 UTC：当前服务器实际安装 `29fe10d`，安装载荷与 DLL 已核对，CI 通过。触发器新建/已有草稿跨页保留、显式保存与取消、条件和行为搜索及键盘提交已实测；英文各主页面切换正常。此前的安装器响应、安装取消恢复、排序按钮边界、时间精度、设置页后台采样状态同步及共享 Runtime 保留均已有对应候选证据。

空闲采样在 `29fe10d` 仍出现一次警告后恢复，因此未判通过。`e28111d` 补充刷新未完成 IPC 的服务观察状态，128 项相关回归通过，服务器已打包但未安装。实测另发现英文节点地区文字截断、内置配置仍启用重命名/删除；源码已修复，18 项目构建零警告零错误、完整主程序 **3,137 / 3,137**、格式检查通过，等待合并候选安装复验。两个安装器并发修复时一方显示笼统提权失败，仍需追查。

当前矩阵 **66 项通过、27 项部分完成、48 项待测**；完整多语言与 RTL、小窗口/键盘/托盘、采样统计与清理、安装器剩余故障场景和临时资源回收尚未完成。不能作为完整发布验收。本地与远端仅 `main`、开放 PR 为零的最近核对时间为 10:11 UTC。生产设置权威切换仍是独立未完成事项。
