...
9 月 23 日继续验收：`b3ca172` 已在服务器全新安装，规则列名与比例列宽在最大化和约 878×684 的窗口下显示正常；空名称重命名会留在弹窗内提示，补填名称后保存成功，持久化名称已核验。该提交的构建、格式、测试和离线安装包 CI 全部通过。`0872a3c` 的正文下载超时、取消、重试、历史恢复、无效导入及删除证据继续保留；当前配置删除后切回内置直连并能从原订阅重新导入，其他模式下的删除仍待测。当前矩阵为 50 项通过、20 项部分完成、71 项待测。累计流量漏计、其余交互矩阵及资源清理仍未完成，不能作为完整发布验收。
