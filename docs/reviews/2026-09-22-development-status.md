...
9 月 23 日继续验收：`9dfbd39` 的 App 核心流量累计、停止及退出收尾、重启不重复入账，以及服务 TUN 下载和空连接列表已验证。`6c38c1e` 实机确认名称、通知正文和阈值直接点保存正确持久化；空名称、重名和负阈值被拒绝。重启后启用的进入程序任务执行一次，通知中心显示指定内容，禁用任务未执行。`d4d94f0` 已在服务器全新安装并核对载荷；列表开关、四类参数下拉框和选项弹窗的修复已实机复验，60.5 秒参数、条件与行为排序以及确认删除均核对数据库。3,115 项主程序测试、格式检查及 CI 通过。触发器执行日志缺失、空日志类别刷新后标题空白仍待修复。当前矩阵为 56 项通过、26 项部分完成、59 项待测；采样开关、清空历史、其余交互矩阵及资源清理尚未完成，不能作为完整发布验收。本地与远端仍仅有 `main`，开放 PR 为零。
