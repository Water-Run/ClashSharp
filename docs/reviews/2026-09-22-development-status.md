...
9 月 23 日 13:53 UTC：当前服务器安装 `ef6bfa7`，CI 35868207746 通过，安装 DLL 与候选载荷散列一致。此候选完成“修复中断且应用包缺失”的真实恢复：受控移除冲突包后，在管理程序点击继续修复即可重新部署并清除事务，未手工注册载荷。验收数据另从完整备份恢复，不计自动升级保留数据通过。
...
用户追加首页磁贴专项：推荐布局从 8 张扩展到 20 张，可选磁贴从 58 张增加到 68 张。8222053 已实测应用/服务归属内存和启动时间、完整订阅用量与到期时间、IP 查询、网站检测及模式切换清空旧检测。ef6bfa7 修复编辑器裁切、拖动入口、内核状态和多行摘要，正在实机复验。后续又修复停止内核后返回首页丢失会话流量，18 项目构建零警告零错误、完整主程序 **3,166 / 3,166**、格式检查通过，此项尚待下一包。完整范围与限制见[磁贴扩充与验收](2026-09-23-dashboard-tiles.md)。
