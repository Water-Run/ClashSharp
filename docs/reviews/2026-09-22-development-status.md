...
9 月 23 日 16:22 UTC：当前服务器安装 `0492ead`，CI 35885031427 通过，安装 DLL 与候选载荷散列一致。227cb08、3d5bad4、3e30bec、0492ead 四轮正常卸载后均核实包/服务及六处自有目录消失，随后正常安装成功；验收数据另从关闭后的散列备份恢复，不计自动升级保留数据通过。此前 ef6bfa7 已通过“修复中断且应用包缺失”的真实恢复，未手工注册载荷。
...
用户追加首页磁贴专项：推荐布局从 8 张扩展到 20 张，可选磁贴从 58 张增加到 68 张。已实测编辑器最小/恢复/最大窗口、取消与搜索、拖动后进程重启保留、键盘跨行遍历及详情、停止内核后跨页面保留会话流量。新增订阅余额/超额、单向已知流量及过期详情已通过完整/部分/缺失/超额/过期实测。新增批量选择和计数、波斯语编辑器、隐藏全部后重启及恢复推荐保存通过；全部 68 项保存及正常重启后保留、启动引导 RTL 已在 0492ead 复测通过。另发现引导深色遮罩使用旧模板、统计磁贴数字缺少说明，已修源码和七语文案，等待新包复验。相关回归 496 / 496，构建零警告零错误，格式检查退出 0（有工作区加载警告）。完整范围与限制见[磁贴扩充与验收](2026-09-23-dashboard-tiles.md)。

当前矩阵 **68 项通过、31 项部分完成、42 项待测**；完整多语言与 RTL、小窗口/键盘/托盘、采样统计与清理、安装器剩余故障场景和临时资源回收尚未完成，不能作为完整发布验收。14:44 UTC 再核对本地与远端仅有 `main`，开放 PR 为零。生产设置权威切换仍是独立未完成事项。
