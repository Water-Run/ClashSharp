...
已按用户要求收尾并暂停开发和验收；恢复入口及未测项见[实机验收记录](2026-09-22-server-ui-acceptance.md)最新章节。

## 当前状态（9 月 24 日暂停交接）

本地与远端仅保留 `main`，开放 PR 为零。`48fb0dc` 的 CI 两项作业通过，已在目标服务器正常安装并验证载荷散列。原始中断恢复、提供方服务/TUN 启动、小窗口反馈已有原生证据；最新 HTTP 503 专属错误与零条目警告也已复验。验收数据由备份人工恢复，不计自动升级迁移。

功能矩阵为 **80 项通过、37 项部分完成、24 项待测**。新增活动连接刷新、TUN 磁贴与实际进程/系统代理/网卡对应、核心版本与运行时长证据。提供方更新会使末行滚出视口，源码现改为保留集合和原行并更新值，按钮保持可聚焦，请求归页面生命周期管理。18 项目构建零警告零错误，最终主程序 **3244 / 3244**、无跳过。此修复尚未制作和安装新候选，恢复工作后先做原生滚动与焦点复验。内核接受部分损坏规则文件的解析行为仍开放；完整发布验收、磁贴全部状态/语言/主题组合及生产设置权威切换仍未完成。

后续还需按用户新增要求清理安装器样式：参照 Windows 常见安装维护流程整理布局和控件层级，减少装饰与冗余标语，保留必要状态、操作和诊断信息，并进行真实截图验收。模式磁贴固定引用规则模式说明的代码问题也已记录待查。

下文保留历史阶段记录，其中候选版本、计数和“待验”状态以本节及最新追加记录为准。

## 历史阶段记录

9 月 23 日 19:42 UTC：当前服务器安装 `59f193d`，[CI 35906610261](https://github.com/Water-Run/ClashSharp/actions/runs/35906610261) 通过，安装 DLL 与候选载荷散列一致。45d534c 正常卸载后核实包/服务及六处自有目录消失；59f193d 正常安装成功，验收数据由闭合散列备份手动恢复，不计自动升级迁移。正常/最小窗口各连续回车切换待机/关闭，模式按钮焦点保持已复验；内存详情去重、内核内存/运行时长/启动方式此前已实测。基础设置重置取消/确认、测速 URL1 校验和还原保存/取消新增实机证据。新发现兜底恢复注册仅写 MSIX 私有注册表，界面假成功；已改用独立 Windows StartupTask，设置/引导/磁贴异步读取实际状态，18 项目构建零警告零错误，3180 项主程序测试通过、无跳过，格式检查通过。新注册方案等待候选安装后复验。
...
20:36–20:55 UTC：当前安装 `b10deb1`，[CI 35915168188](https://github.com/Water-Run/ClashSharp/actions/runs/35915168188) 通过，安装 DLL 与载荷散列一致。设置页注册、检测及连续十次回车均保留焦点。恢复程序单独执行 1.646 秒后退出，返回 0，六次窗口采样均为零、没有内核；真实注销/登录也正确传入恢复参数并退出。主程序单独自启正常打开窗口，两项同时启用的本次登录按主程序先启动顺序正常，未留下重复实例。随后补测相反顺序复现缺陷：恢复程序先启动、150 毫秒后启动主程序时，两个进程都退出，主窗口丢失。现按角色分开实例标识，主程序等待恢复结束后创建宿主，恢复程序不再激活已有主窗口；构建零警告零错误，原生复测待下一候选。实际代理残留恢复仍待测，不能由无残留场景推定通过。

21:36 UTC：`aa9a467` 安装及 CI 通过。恢复程序先启动后分别延迟 150/700 毫秒启动主程序，均保留一个正常窗口；已有主程序和内核不受恢复程序影响。实际本地端口残留清理及外部端口代理保留已验证并还原测试账户。另发现关闭残留检查后退出路径仍清理代理，已改为恢复宿主只收尾、不再次改变网络；3191 项完整回归与格式检查通过，待下一候选原生复验。详见实机记录最新章节。

20:33 UTC 当时矩阵为 **72 项通过、34 项部分完成、35 项待测**；完整多语言与 RTL、小窗口/键盘/托盘、采样统计与清理、安装器剩余故障场景和临时资源回收尚未完成，不能作为完整发布验收。当时再核对本地与远端仅有 `main`，开放 PR 为零。生产设置权威切换仍是独立未完成事项。
...
### 760de49 登录恢复、安装载荷故障与测速反馈（21:37–22:07 UTC）

760de49 在 Windows Server 2025 正常安装，DLL SHA-256 `1B7333380B256C61CDE2BEC06AD160308EEB312A46301AC64AF1A3BA73C9A91B` 与载荷一致，仅验收 SID 注册；安装器、MSIX 和运行库签名均为 Valid。[CI 35923572344](https://github.com/Water-Run/ClashSharp/actions/runs/35923572344) 两项检查通过。关闭代理残留检查后，恢复程序 1.386 秒退出，5 次窗口/内核采样均为零，ProxyEnable=1 和端口 10000 保持原状。重新开启后，仅注册恢复程序并真实注销/登录到会话 7：Windows 于 21:52:45 自动启动 PID 4984，恢复步骤 434.353 毫秒后请求退出，实际残留被关闭，无警告/错误；启动主程序前无应用/内核剩余进程。每次均还原验收账户原始代理值。恢复注册、执行与开关语义的原生复验完成。

签名安装器的独立副本分别去掉运行库文件、翻转 MSIX 第 4096 字节，均在部署前拒绝，包/服务/六处目录仍为零。此证据只覆盖缺失文件与损坏载荷，不能代替干净机器离线依赖安装或错误签名证书。原生主状态只显示泛化阻止原因，现改为直接说明校验失败或安装文件不完整，并给出获取同版本完整包、完整解压的处理步骤；152 项安装器呈现回归通过，新文案尚待换包复验。

磁贴测速实测只报告节点数量；节点页显示数字 1，刷新后消失（`215810Z-532405619`、`215907Z-114051914`、`220011Z-663620803`）。已补逐节点 TCP 测量结果、毫秒单位、未测试/失败/无法测量/直连状态；本次应用运行内保留最近完整批次，并按名称、协议、主机和端口匹配，避免新地址复用旧值。取消中的探测不会作为失败写入；无当前节点时磁贴显示不可用。七语说明明确 TCP 连接耗时与网站响应不同。18 项目构建零警告零错误（36.88 秒），完整主程序回归 3204/3204、无跳过（1 分 13 秒），完整格式检查通过；新界面尚待原生复验。

38 个截图/动作文件已归档并验证 SHA-256 `86F90A1297298AD068A81A8E5D73CE23F59F413F33663F6625FF4715F6DCBF14`（`desktop-evidence-20260923-2135-to-2200.tar.gz`）。当前功能矩阵 73 通过、37 部分完成、31 待测；磁贴所有语言/主题和全部动作、完整安装器故障场景仍未完成。22:06 UTC 检查本地及远程仅 main，无开放 PR。
