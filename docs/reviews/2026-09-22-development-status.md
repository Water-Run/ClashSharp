...
9 月 23 日继续验收：`1924e61` 候选已验证首页实时刷新、显示编辑器、磁贴编辑器及不兼容备份提前拒绝；TUN、HTTP/SOCKS5 和原生系统代理已完成受控流量检查。新发现全局模式显示错误节点、切换 TUN 丢失节点选择及累计流量漏计。节点显示源码修复已完成，18 项目构建零警告/错误、主程序 3029 项和相关 270 项回归通过，格式检查通过；新包实测、节点持久化、完整累计统计及其余交互矩阵仍继续，不能作为完整发布验收。
