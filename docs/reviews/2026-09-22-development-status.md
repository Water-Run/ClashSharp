...
## 当前状态（9 月 23 日 22:49 UTC）

本地与远端仅保留 `main`，开放 PR 为零。`0301dab` 的 CI 两项作业通过，并在目标服务器正常安装、验证载荷散列后完成本轮验收；目前已正常卸载，包、服务和六处产品目录均为空，准备安装后续修复候选。应用关闭后的 32 个验收状态文件已备份并记录散列，后续人工恢复不计自动升级迁移。

功能矩阵为 **76 项通过、35 项部分完成、30 项待测**。测速磁贴已实测 10 节点成功/拒绝/超时/直连、完整长名称、鼠标滚动、取消/重试、刷新及跨页保留；最小窗口发现结果列表不能通过 Tab 进入，已修复待复验。节点名称 `node` 误识别为德国也已修复，12 项回归覆盖普通单词、编号地区、名称优先和订阅 URL 路径；完整主程序回归 **3216 / 3216**、无跳过，最终 18 项目构建零警告零错误。760de49 卸载保留外来文件、0301dab 损坏载荷/缺失依赖的明确提示及正常卸载完成提示均已有实机证据。完整发布验收、磁贴全部状态/语言/主题组合及生产设置权威切换仍未完成。
