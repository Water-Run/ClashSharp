...
9 月 23 日继续验收：`ccea7ce` 已在目标服务器验证节点显示、TUN 开关及正常重启后的选择保持，六份实际网络探测确认全局代理与规则直连符合各自选择。`5d904d2` 实测修复订阅输入反馈、无选择按钮和初始日期显示，并通过 401、404、无效/空响应、离页取消、超时恢复及重定向检查；删除订阅保留已导入配置。`656418c` 修复首页并发读取误报不可用及自动更新关闭时手动更新失败，两项 CI 通过，正在安装后复验。订阅用量和到期元数据及带标签的可换行布局已补入源码，主程序 3077 项回归通过，仍待新包实测。累计流量漏计、完整订阅生命周期、其余交互矩阵及资源清理仍未完成，不能作为完整发布验收。
