...
9 月 23 日继续验收：`0872a3c` 已在服务器全新安装，实际验证正文下载约 30 秒超时、离页取消、恢复地址后重试成功，以及历史标签、恢复结果提示、无效配置拒绝和非当前配置删除。`4d9b426` 已实际加载 180 个节点、301 条规则并滚动至末尾。当前矩阵为 50 项通过、17 项部分完成、74 项待测。规则页缺少列名及空名称重命名关闭弹窗的问题已修复，18 项目构建、537 项相关回归和完整格式检查通过，等待新候选安装复验。此前的节点选择保持、订阅元数据和安装修复恢复证据继续保留。累计流量漏计、其余交互矩阵及资源清理仍未完成，不能作为完整发布验收。
