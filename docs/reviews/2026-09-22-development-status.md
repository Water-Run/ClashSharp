...
9 月 23 日继续验收：`1924e61` 候选已验证首页实时刷新、显示编辑器、磁贴编辑器及不兼容备份提前拒绝；TUN、HTTP/SOCKS5 和原生系统代理已完成受控流量检查。新发现全局模式显示错误节点、切换 TUN 丢失节点选择及累计流量漏计。节点显示与按配置保存选择的源码修复已完成，18 项目构建零警告/错误、主程序 3049 项回归和格式检查通过。订阅页也复现了空输入无反馈与无选择按钮误启用问题。新包实测、订阅交互、完整累计统计及其余交互矩阵仍继续，不能作为完整发布验收。
