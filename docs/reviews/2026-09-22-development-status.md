...
9 月 23 日 12:05 UTC：当前服务器实际安装 `5ae3ac6`，安装载荷与 DLL 已核对，CI 通过。内置配置重命名/删除按钮已正确禁用，英文节点地区文字完整显示。空闲采样修复经过首次启动和设置页重启两次实测，均无 ConnectionSampling 警告、连接或虚假流量记录。该批 18 项目构建零警告零错误、完整主程序 **3,137 / 3,137**、格式检查通过。
...
用户追加首页磁贴专项：推荐布局从 8 张扩展到 20 张，可选磁贴从 58 张增加到 68 张，增加内核内存/运行时间/运行归属、系统信息、代理地址、订阅用量/到期/更新时间、公网出口 IP 和更新检查，并把网站测试接入首页直接执行。已有磁贴加入完整详情展开、流量趋势和恢复推荐布局。专项范围见[磁贴扩充与验收](2026-09-23-dashboard-tiles.md)。18 项目构建零警告零错误、完整主程序 **3,161 / 3,161**、完整格式检查通过；新包尚未实机验收。

当前矩阵 **68 项通过、31 项部分完成、42 项待测**；完整多语言与 RTL、小窗口/键盘/托盘、采样统计与清理、安装器剩余故障场景和临时资源回收尚未完成，不能作为完整发布验收。11:50 UTC 再核对本地与远端仅有 `main`，开放 PR 为零。生产设置权威切换仍是独立未完成事项。
