...
## 当前状态（9 月 23 日 23:44 UTC）

本地与远端仅保留 `main`，开放 PR 为零。`d6f64f2` 的 CI 两项作业通过，已在目标服务器正常安装并验证载荷散列。磁贴测速结果的 Tab/PageDown/PageUp/Esc 和节点地区识别修复已完成原生复验。验收数据由备份人工恢复，不计自动升级迁移。

功能矩阵为 **77 项通过、37 项部分完成、27 项待测**。代理提供方的成功更新、HTTP 503、无效 YAML 后保留旧数据及重试已有实际请求和界面证据；规则提供方已验证更新数量，缺失时间显示正确。服务模式使用该配置时暴露启动恢复死锁，旧候选只能人工修复验收状态；此项不计通过。已补严格受限的中断恢复、提供方路径规范化、可信拒绝后的无提权清理、小窗口时间列及可见操作反馈，等待新安装包复验。18 项目构建零警告零错误，123 项定向回归和完整主程序 **3234 / 3234**、无跳过，完整格式检查通过。完整发布验收、磁贴全部状态/语言/主题组合及生产设置权威切换仍未完成。
