...
## 当前状态（9 月 24 日 00:18 UTC）

本地与远端仅保留 `main`，开放 PR 为零。`0f3aaa8` 的 CI 两项作业通过，已在目标服务器正常安装并验证载荷散列。原始中断状态重放后自动恢复成功，提供方配置在服务/TUN 模式启动正常，小窗口时间列及离屏反馈修复完成原生复验。验收数据由备份人工恢复，不计自动升级迁移。

功能矩阵为 **79 项通过、37 项部分完成、25 项待测**。新增实际 DIRECT/代理规则命中及连接页离开后返回的证据。服务模式 HTTP 503 保留旧数据、重试成功，但错误文案误导为内核不可用；现已改为提供方错误。内核接受部分损坏规则文件并生成零条规则，界面现补零条目警告及结果缺失检查，尚未修复内核解析行为。18 项目构建零警告零错误，完整主程序 **3242 / 3242**、无跳过，完整格式检查通过；本轮新反馈等待候选原生复验。完整发布验收、磁贴全部状态/语言/主题组合及生产设置权威切换仍未完成。
