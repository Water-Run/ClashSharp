...
该项仍未最终关闭：WPF production composition 尚未接入，Rust 当前发布 authority 也尚未持久记录/释放证书所有权；Linux 交叉编译不能证明 CurrentUser store、用户 SID、ACL、UAC 或真实卸载行为。必须等 Windows adapter E3、差分迁移和 E4 的 pre-existing/owned/collision/cut-point 矩阵通过后才能关闭 P0-03。
...
2026-08-30 组合根复审进一步发现一项必须先修的 authority mismatch：WPF manifest 固定 `asInvoker`，protected root 的 exact target-user ACE 又刻意只有 `ReadAndExecute`，但当前 `InstallerCoordinator` 仍在首次 UAC 前直接 `SaveAsync(Prepared)`、在 helper 回执后写 parent mirror、最终直接 clear。把它接到 `WindowsInstallerProtectedStateStores.Transactions` 后，普通用户写入必然被 ACL 拒绝；若 helper 已提交同一后继，parent 再写也会成为双写/CAS 冲突。因此“store 已实现”不能表述为“production coordinator 已可直接组合”。关闭方式固定为不放宽 ACL：parent 只构造首个 canonical Prepared、只读 reload/exact-compare；authenticated persistent helper 在任何副作用前唯一持久化 Prepared，并唯一拥有 journal/ledger 的 transition 与 terminal clear。

OTS 证书边界也已查明：当前 `WindowsCurrentUserCertificateStoreAdapter` 依赖进程 token 的 `StoreLocation.CurrentUser`，并在打开 store 前要求该 token SID 等于 `request.TargetSid`；因此另一管理员凭据启动的 helper 会 fail closed 为 target-user mismatch，不会静默写错库，但也无法完成目标用户 mutation。微软文档说明 `CERT_SYSTEM_STORE_USERS` 可用文本 SID 打开指定用户 store，因此首选实现是 helper 通过原生 `CertOpenStore` 只打开 `<exact target SID>\TrustedPeople`，继续执行 thumbprint + DER SHA-256 双重身份策略；必须在 Windows 11 x64 的标准用户 profile + alternate-admin OTS 场景证明后才能启用。当前 pipe 中 parent 是 server、helper 是 client，而 `ImpersonateNamedPipeClient` 只允许 server 冒充 client，所以它不能让 helper 获得 parent 用户身份。若 target-SID store 路径无法取得真机证明，回退方案只能是 helper ledger cut-point 包围 parent-side cert mutation，不能误写管理员 CurrentUser 或放宽 protected root。依据与实现门禁见 [Installer machine-helper IPC boundary](../design/2026-08-30-installer-machine-helper-ipc.md)。

权限分工已先落下一个编译期 primitive：新增只含 `LoadAsync` 的 `IInstallerTransactionReader`，writer `IInstallerTransactionStore` 继承它；Windows protected stores 同时暴露 `TransactionReader` 与 elevated `Transactions`，并由现有共享 root/lifetime 测试证明两者是同一 guarded store 的不同能力视图。当前 coordinator 仍接受 writer 并调用 Save/Clear，因此 production composition 仍未闭环，不能把新增 reader 误报成已完成重构。

helper-only package commit 的 OTS 查询路径也完成了接口级推进。微软 `PackageManager.FindPackagesForUser(userSecurityId, packageFamilyName)` 契约明确：空 SID 查询当前用户，指定另一 SID 需要管理员权限。Windows facade 已从硬编码 current-user 改为显式接收 SID；现有 parent adapter 继续传 `string.Empty` 且测试锁定不改变行为，后续 elevated helper 必须传 journal 的 exact `TargetSid`，再复用 one-registration/full identity/health fail-closed policy。当前尚未写 helper inspector 或取得 alternate-user AppXSVC 运行证据，因此只记为接线 primitive，不记为 CommitPackage 闭环。
...
为准备独立 NativeAOT helper，transaction、association、command 与 result codec 已移除反射式 `JsonSerializer` 依赖并改用显式字段 writer/parser；WPF 外壳同时设置 `StartupHookSupport=false`，且当前 managed WPF entry point 对 helper grammar 仍固定返回 fail-closed exit code。新增的 `runas` launcher 只接受绝对路径且文件名必须是 `ClashSharp.Installer.MachineHelper.exe`，使用 STA、`UseShellExecute=true`、固定 `ArgumentList`，并把 Win32 1223 明确映射为 UAC 用户取消。当前又写入独立于 journal command identity 的严格八参数 parent-PID bootstrap、protected pipe DACL、first-instance/单实例 server 及 `GetNamedPipeClientProcessId`/`GetNamedPipeServerProcessId` 双向精确匹配 primitive。DACL 保留 exact logon SID 以覆盖同账户 split-token consent，同时加入仅 ReadWrite 的 Builtin Administrators 以覆盖标准用户输入另一管理员凭据的 OTS elevation；后者只是连通性门，不替代连接后 PID 认证。默认 Everyone/Anonymous/Interactive/AuthUsers 仍不允许。设计见 [Installer machine-helper IPC boundary](../design/2026-08-30-installer-machine-helper-ipc.md)；这些源码尚未通过最新资源门后的编译，也未接成 broker。签名且 hash-locked 的 NativeAOT helper、one-EXE 内嵌/安全提取、父子 Authenticode 绑定、persistent broker、SCM/ACL mutator 和 helper 终止证明仍是未闭环项，不能用当前 primitive 代替。
...
OOM 冷却期结束且每次任务前标准资源门禁通过后，最近一个已验证 Core checkpoint 已在锁定的 .NET SDK 10.0.201、Linux `net10.0`、Release、单 MSBuild 节点下完成 499/499，0 skipped。首次重建 checkpoint 时依次暴露并修复 MSIX verifier 的 CA1068、manifest 测试遗漏 machine-files 构造参数，以及一个“跨 command”负例实际重复生成同一确定性 command 的假负例；修正 transaction identity 后，该绑定测试才真正生效。后续又补齐 `CommitPackage`、每个 committed boundary 的 ack-loss、replay postcondition drift，以及命令间 protected state 偷跑/丢失矩阵。该 checkpoint 的 Cobertura 为 line 94.90%（3108/3275）、branch 85.67%（1495/1745），超过 CI 的 line 90% / branch 80% 下限；安全关键的 `InstallerMachineHelperSessionGuard` 为 line 89.83%、branch 82.40%。之后新增的 strict bootstrap 与跨语言 service pipe-name 固定向量使当前 Core 静态展开计数成为 512，尚未编译或执行，不能继承这份覆盖率证据。

MSIX verifier 已下沉为跨平台 Core 边界，当前静态定义 48 个展开用例实例：两个 canonical success/stream-ownership 场景，以及主包/应用身份、PackageIntegrity、依赖声明与框架身份、必需 ZIP 条目缺失/改名/重复、非法 UTF-8、DTD、重复 XML 元素、non-seek stream 和 cancellation 的负例矩阵；新增矩阵逐一覆盖 7 项 machine file 缺失与逐项 hash 篡改、额外 service/GeoData 项和大小写碰撞。release-manifest 测试另锁定固定 7 项集合、ordinal 顺序、Windows 歧义路径、GeoData/executable/combined 边界以及 machine JSON 的未知、重复、错大小写和错类型属性。测试直接调用生产 parser/codec，不再只依赖 Windows locker 间接覆盖；当前仍仅为“测试已写”，须以资源门放行后的实际发现数和结果为准。

当前静态展开用例计数为：`ClashSharp.Installer.Tests` 512 个、`ClashSharp.Installer.Presentation.Tests` 70 个、`ClashSharp.Installer.Windows.Tests` 103 个；三者均无 `MemberData`/`ClassData`、无 skip，也没有固定 sleep。最近实际跑绿的是 bootstrap 变更前的 Core 499 项与 Presentation 39 项；Presentation 的 router 签名随后改为 strict bootstrap，package/journal 单卡片策略现按 15 个合法 operation/phase 组合逐项证明 durable recovery 优先，并把 5 个非法 state/operation 组合拆成独立报告，同时将 `CanExecute=false` 对 Available/Installed/三种 Recovery 全部展开；故当前 512/70 源码都需重新验证。Core 新增用例还以 Rust Installer 的既有固定向量证明 association 派生出的 service pipe name 与 App/Service C# 协议一致，并验证输出不暴露 SID/token、对 SID/token 变化敏感。Presentation 现在从 canonical installed version 与 exact protected transaction 只产生 Available、Installed、RecoveryRequired 三态；任一 durable phase 优先并携带 exact resume operation，坏 version/snapshot 或状态-操作错配均 fail closed。它还继续锁定 busy 覆盖态、单卡片精确按钮集合、fatal exception 传播、命令门释放、pre-WPF 三分支行为路由，以及 SVG/WPF 的 hexagon、shadow、white mark 三层 exact geometry。Windows 集合除真实 Win32 sharing、目录 rename、payload、MSIX identity/framework、CurrentUser certificate/package 与 PowerShell→C# 清单互操作外，还增加 journal-bearing elevated adapter、STA `runas` 参数边界、UAC 1223、可恢复/致命错误分类、相对/UNC/device/noncanonical helper 路径拒绝、install/repair/uninstall 三条 package-phase helper commit、committed replay postcondition failure、16 项 protected-root/ACL/lease/composition 矩阵、9 项 pipe ACL/first-instance/PID primitive 矩阵，以及 10 项 exact-target-SID package commit 后置条件矩阵。Windows 生产项目与前 76 项 checkpoint 已在 Linux 完成 Windows 11 x64 Release 交叉编译，0 warning/0 error；最新 103 项源码 checkpoint 被资源门阻止启动，且 Windows 测试从未在 Linux 执行，仍待 Windows CI/E3。

WPF 单卡片项目在拆分后已完成 Windows 11 x64 Release 交叉编译，0 warning/0 error；这只验证项目/XAML 编译边界，不代表 Linux 执行过 WPF runtime 或 UIA。Rust Installer 因 Linux 缺少 `fontconfig.pc` 且当前动态加载组合不兼容，未在本机执行测试；已用单 Cargo job 和现成 `x86_64-pc-windows-msvc` target 完成 `cargo clippy --tests --locked -- -D warnings`。这些证据至多是 E2，不能外推到 Windows runtime、PowerShell/certificate mutation 或签名 VM。
...
7. Rust/C# differential vectors 保留到正式切换；
...
| 项目 | 目标责任 | 当前状态 |
|---|---|---|
| `ClashSharp.Installer.Core` | `net10.0` immutable request/journal、strict codec、operation-specific state machine、certificate ownership、manifest/budget、sanitized result、平台端口 | MachineReserved 顺序、canonical journal digest、无路径 helper command/result/framing/two-phase session guard、helper-only package commit、证书 Core、Windows 11 x64 policy、locked-release、包内 7 项 machine hash anchor 与 package policy 已通过 499 项；strict parent-PID bootstrap 与跨语言 service pipe-name 向量已写，当前 512 项待资源门后重验；coordinator 仍需从 protected-store writer 改为 helper-authoritative transition + parent read-only reload，SCM/elevation/final composition 仍未实现 |
| `ClashSharp.Installer.Presentation` | `net10.0` 单卡片状态机、命令、readiness/result 边界 | 已从 WPF 拆出；最近 39 项 Linux 契约测试全绿，strict bootstrap router 与 package/journal 三态 policy 增量待重验 |
| `ClashSharp.Installer.Presentation.Tests` | 直接引用 Presentation 的状态/并发/异常、startup routing 与 executable contract 测试 | 最近 39/39、0 skipped；当前静态 70 项把 exact recovery 展开到全部 15 个合法 durable phase、5 个非法 state/operation 组合及全部 blocked state，尚未重跑 |
| `ClashSharp.Installer` | `net10.0-windows10.0.22000.0` WPF 单产品卡片、UIA、高 DPI、Windows platform probe、内嵌清单 loader | shell 与 formal-resource 契约已落地并曾完成 Release 交叉编译；strict bootstrap 依赖增量待重编，runtime 仍明确 fail-closed |
| `ClashSharp.Installer.Tests` | 直接引用 Core 的 pure contract 与 coordinator tests | 最近 checkpoint Release 499/499、0 skipped、line 94.90% / branch 85.67%；当前静态 512 项含 bootstrap 矩阵和跨语言 service pipe-name 固定向量，尚未重验 |
| `ClashSharp.Installer.Windows` | SafeHandle payload lease、CurrentUser certificate、MSIX metadata/machine hash、current/target-user PackageManager、protected-state root 与后续 SCM/ElevationBroker | root/store、strict bootstrap、pipe DACL/first-instance、双向 PID primitive 与 exact-target-SID package commit inspector 已写；首个 76 项 checkpoint Release 交叉编译通过，最新 103 项 checkpoint 待资源门；signed helper、persistent authenticated broker、machine mutation 仍未完成 |
| `ClashSharp.Installer.Windows.Tests` | Windows adapter/integration 与 PowerShell→C# build-contract 互操作 | 当前静态展开 103 项；前 76 项程序集 Release 交叉编译 0 warning/0 error，新增 27 项待重新编译；Windows CI 实际执行待验证/E3 |
| VM harness | 签名 candidate 的真实 fault/reboot matrix | 未开始 |

当前 C# Core 与 Rust journal 都已把 operation 纳入 immutable identity。Rust 仍以 helper 核验后写入的 `Prepared` 充当卸载 tombstone；C# 未发布 schema 则进一步显式区分 `Prepared` 与 `MachineRemovalAuthorized`，只有后者可进入 Remove。Rust helper 已在 association 删除前持久化受保护 authorization tombstone，并在 package absence 与 machine absence 双重核验后才清除；C# protected root/store、strict bootstrap、pipe security/PID primitive、service pipe-name parity、target-SID package commit inspector 与单卡片三态决策已写，但尚未接入 authenticated helper broker，coordinator 的 protected writer 权限模型也必须先改为 helper-authoritative，且最新 512/70/103 源码 checkpoint 待资源门重新编译。P0-02 已完成状态机、协议及 protected-root/IPC primitive 级推进，但必须等 broker、Windows ACL/SCM/cut-point E3/E4 才能最终关闭。P0-03 的 C# ownership protocol、durable store、coordinator order、pure fault matrix 和 Windows `X509Store` adapter 已实现；WPF/helper production composition 与 Rust authority 的 ledger/remove 仍未实现，因此同样保持未闭环。
...
| 阶段 | 内容 | 退出证据 |
|---|---|---|
| M0 | strict v2 journal、package version、operation order、uncertain outcome、Rust/C# differential vectors | Linux Core tests；未知/重复字段与完整 phase matrix 全过 |
...
| M6 | 切换 authority 并移除 Rust/Slint | E4 完整，回滚手册和旧 pending journal recovery 已验证 |
