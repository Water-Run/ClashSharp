...
transaction、association、command 与 result codec 已移除反射式 `JsonSerializer` 依赖并改用显式字段 writer/parser；WPF 外壳同时设置 `StartupHookSupport=false`。managed entry point 会在创建 WPF `Application` 前解析 helper grammar、加载唯一内嵌 manifest，并进入 `WindowsInstallerMachineHelper`；非法或不可恢复 bootstrap 以稳定非零退出码结束。按“WPF 绿色单文件”最终方向，`runas` launcher 只接受绝对 canonical `ClashSharp.Installer.exe`，复用同一已签名映像的 helper 分支；它使用 STA、`UseShellExecute=true`、固定 `ArgumentList`，并把 Win32 1223 明确映射为 UAC 用户取消。
...
| 项目 | 目标责任 | 当前状态 |
|---|---|---|
| `ClashSharp.Installer.Core` | `net10.0` immutable request/journal、strict codec、operation-specific state machine、certificate ownership、manifest/budget、sanitized result、平台端口 | helper-authoritative coordinator/session、helper-only package commit、certificate ledger、schema-2 locked release 与 fault matrix 已完成；Release 547/547 |
| `ClashSharp.Installer.Presentation` | `net10.0` 单卡片状态机、命令、readiness/result 边界 | strict startup router、精确 allowed-operation、三态恢复优先、single-flight/cancel/fatal exception 边界已完成；Release 88/88 |
| `ClashSharp.Installer.Presentation.Tests` | 直接引用 Presentation 的状态/并发/异常、startup routing 与 executable contract 测试 | 88/88、0 skipped；覆盖 production runtime、受限平台仅卸载、关闭取消与 runtime lifetime |
| `ClashSharp.Installer` | `net10.0-windows10.0.22000.0` WPF 单产品卡片、UIA、高 DPI、Windows platform probe、内嵌清单 loader | C# 紫色单卡片 shell、helper 路由与 gated production composition 已落地并完成 Release 编译；默认产物仍使用 fail-closed preview runtime |
| `ClashSharp.Installer.Tests` | 直接引用 Core 的 pure contract 与 coordinator tests | Release 547/547、0 skipped；旧 499 项覆盖率需在 promotion 前重采 |
