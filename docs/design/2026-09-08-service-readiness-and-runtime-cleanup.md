...
运行 `7db1fcadaa7348a180f42247966d5f20` 继续使用同一 `c0495f4` 签名入口和 MSIX。
安装成功后，独立客体进程以只读权限打开实际服务，并跨卸载保持句柄；服务从
Running 变为 Stopped，卸载返回 Uncertain，阶段保留 MachineRemovalAuthorized。
机器载荷逐文件长度和摘要保持一致，精确包、机器证书、物理用户证书、关联及
证书归属记录均保留。该结果没有将 SCM 停止状态当作服务已删除。

释放句柄后，由独立检查进程读取同一阶段，再由新 parent 完成卸载；产品资源
移除快照仍先于兜底清理。九项客体清理全部通过，未观察到服务崩溃事件，客体
`1e7799bd-0fc7-4d6c-aa56-a0c6e8be4dc7` 已销毁，输入和开发机代理未变。
脱敏收据为 `artifacts/verification/installer-engine-delete-pending-m5d.json`。
前一运行虽通过客体检查，但主机收据遗漏完成标记，保留原记录并修复夹具后在
新客体复跑；以前述完整运行作为验收依据。

测试没有捕获 Win32 1072 返回码，因此只证明实际服务句柄占用期间的保留与恢复
行为。其候选来源保持 `c0495f4`，不能标为后来 `027d1c7` 的完整包验收。
