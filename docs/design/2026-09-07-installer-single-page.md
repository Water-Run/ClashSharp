...
| 有准确的未完成事务 | 仅继续对应操作 |
| 操作进行中 | 显示步骤及进度，可请求取消 |
| 检查未通过 | 显示原因与诊断入口，普通操作禁用；专用账户入口须重新检查及确认 |
...
生产运行端口增加独立的“切换使用账户”入口，位于详情区。账户信息由认证 helper 检查，
页面只展示新切换/继续恢复以及保留旧账户应用和配置的说明。确认区仍在同一页面，
提供“暂不切换”和“确认切换并继续”；只有明确选择后才能进入专用迁移。

![实际 WPF 确认区，使用模拟运行端口](assets/installer-owner-transfer-1.0.0.png)

旧账户普通日志无法由当前账户读取时，不会据此授权普通操作；专用入口必须完成自身
候选、账户、目录和双 App 屏障检查。生产编译门关闭的预览版本不显示这个入口。
确认在 UI 调度器上等待，关闭或取消会撤销等待并保留“忙碌”直到后端收尾。

最终离屏检查通过 83 项断言，生成 35 张图，覆盖两种确认、普通和最小尺寸、192 DPI
栅格输出及现有状态。确认区完整位于视口，按钮有可访问名称且不默认接受回车。
真实 UIA、讲述人、跨显示器 DPI 与系统安装仍需 Windows 11 验收。
