...
截至 M4t，专用认证确认入口、连续持有全局与双 App 屏障、六阶段执行及普通安装/修复交接均已接通源码和组合测试。普通入口仍拒绝跨账户覆盖；专用入口受同一个生产 mutation 编译门控制。真实账户、正式签名候选及 Windows 11 安装验收尚未完成，不能从模拟端口联测推定已经通过。

## M4t 专用确认与安装会话交接

独立的 --owner-transfer-helper 启动语法仅携带随机会话、候选 payload 摘要及父进程 PID。它使用与普通事务不同的管道命名空间，保留相同的准确登录 SID DACL、两端 PID、提权、签名映像及父进程账户验证。混合模式、变体和残缺参数不能落入 WPF 或普通 helper。

认证后，helper 在全局独占和候选租约下只读检查。旧 association 的首次有界读取只是定位 SID 的线索；固定机器根及准确关联须独立验证。已有私有事务保留全部原始身份和阶段；新事务的凭据与后续普通事务 ID 只在 helper 中生成。缺失私有目录不会在检查时创建。公开 offer 不包含 profile、凭据或私有日志，用户决定绑定本次会话与新生成的 offer nonce。

单页安装器在“安装详情与诊断”提供“切换使用账户”。用户看到旧账户应用和配置的保留范围，并通过独立确认按钮接受；回车不默认接受。Presentation 仅接收是否恢复这一展示事实，不构造账户或候选身份。取消、超时和关闭都等待确认任务及后端收尾。

确认后，WindowsOwnerTransferAuthorityFactory 依次持有全局独占、候选、旧账户 App 屏障和当前账户 App 屏障。获取屏障后再次解析两个 profile；目标证书与候选冲突、确认期间私有状态变化、普通未完成事务及新迁移整树检查失败均在私有 Prepared 写入前拒绝。新准备检查只读，不发布普通启动屏障。

同一作用域执行全部阶段、验证并清除私有日志后，才创建新账户的普通存储。普通 Prepared 的准确身份经过 helper 和 parent 分别复核；同一管道、提权进程和签名映像租约交给普通安装协调器。恢复保留原始 Install/Repair 操作，不能因当前包状态变化重新生成迁移身份。普通协调器继续保留自身环境、降级和包验证。

资源按普通存储、迁移存储、当前 App、旧 App、候选、全局独占的逆序释放。并发关闭等待同一任务，释放失败仍尝试后续释放。普通 broker 的关闭也改为等待真实 helper 退出，移除了后台释放分支，避免迁移后的普通安装阶段提前完成窗口收尾。

本机新增 92 项测试，全量安全集 4073 项通过：主程序 2330、Core 822、Presentation 109、Windows 812。组合测试使用实际协议、双向内存流、helper、迁移执行器和普通事务会话，Windows 副作用与身份探测端口为模拟；覆盖单次提权、拒绝确认、两端认证失败、确认间状态变化、所有持久阶段恢复、取消后的收尾及普通事务最终清除。三个真实证书存储测试继续仅在隔离 CI 执行。

18 项目 Release x64 构建零警告、零错误，format 1347 文件零变更且无工作区警告。实际编译 WPF 窗口离屏生成 35 张图，通过 83 项断言，包括确认区在普通/最小窗口内完整可见、可访问按钮、绑定及取消关闭。没有显示生产窗口、修改本机代理、调用真实迁移/SCM/证书操作；旧账户独立卸载、真实 Windows 11/标准用户与替代管理员完整签名安装矩阵继续推进。
