...
> Development status (2026-09-06): Clash# is undergoing production hardening; source builds and development artifacts do not yet meet production release criteria. The self-contained C# / WPF installer is the only Installer implementation. Its production runtime, parent engine, and authenticated helper are connected in source, but both default and formal packaging keep the mutation-runtime build gate disabled until one signed candidate passes the Windows VM E4 matrix. This development batch reclaims idle subscription-operation locks, adds PowerShell syntax gates for both editions, and fixes the WinUI formatting environment. See the [current development status and execution plan](./docs/reviews/2026-09-06-development-status-and-next-steps.md) for progress and validation, and the [2026-08-31 audit](./docs/reviews/2026-08-31-project-structure-mvvm-code-quality-installer-audit.md) for architecture and Installer authority boundaries.
